...
- **🪵 Logs Estruturados (Pino):** Substituição de consoles comuns por logs estruturados em formato JSON com níveis de severidade (`info`, `error`), integrando perfeitamente com ferramentas modernas de monitoramento (como Datadog, Grafana Loki ou Elastic).
...
- **🎯 IA com Prompt de Alta Fidelidade:** Prompt extremamente guiado no Gemini para assegurar respostas determinísticas de classificação, evitando desvios ou erros de formatação JSON.
...
Para executar a suite completa de testes unitários desenvolvidos com Jest (incluindo testes de classificação para Ouvidoria, Suporte, Financeiro, SAC e Fora do Escopo), utilize:
